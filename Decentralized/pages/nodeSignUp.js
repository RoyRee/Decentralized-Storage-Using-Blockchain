  ...
  event.preventDefault();
    ...
    //setTimeout(this.refresh ,60000);
		...
		try{
			...
			const address = await  factory.methods.nodeSignUp(bytes).send({from:account[0]});
     	// console.log(address);
			this.setState({loading:false});
			Router.pushRoute(`/nodeLogin`);
		}catch(err){
  ...
  <Head>
    ...
    <Grid textAlign='center' style={{ height: '90%' }} verticalAlign='middle'>
      ...
      <Grid.Column style={{ maxWidth: 450 }}>
        ...
        <Message>
         Existing user? <a href='/nodeLogin'>Login</a>                                                                                                                                                                                                                                                                                                           
        </Message>
      </Grid.Column>
    </Grid>
